defmodule Kitto.JobTest do
  ...
  test "#new calls the given job after the given interval" do
    ...
    spawn_link(Kitto.Job, :new, [%{name: :dummy_job, job: job, options: %{interval: interval}}])
  ...
  test "#new, with first_at option, calls job after first_at seconds" do
    ...
    spawn_link(Kitto.Job, :new, [%{name: :dummy_job, job: job, options: %{first_at: first_at}}])
  ...
  test "#new, with first_at unspecified, calls job immediately" do
    ...
    spawn_link(Kitto.Job, :new, [%{name: :dummy_job, job: job, options: %{}}])
  ...
  test "#new, calls jobs multiple times" do
    ...
    spawn_link(Kitto.Job, :new, [
      %{name: :dummy_job, job: job, options: %{first_at: false, interval: interval}}
    ])
